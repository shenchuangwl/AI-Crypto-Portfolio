...
export function OnlyCoinLists({ data }: { data: OnlyCoinDaily }) {
  return <>
    ...
    <div className="onlycoin-lists">{(['long', 'short', 'onlycoin'] as const).map(key => <div key={key}>
      ...
      <ul>{data[key].map(member => <li key={member.member_id}>
        <Link to={`/market/${encodeURIComponent(member.symbol)}?from=screener-y&direction=${member.first_direction === 'SHORT' || member.first_direction === 'down' ? 'down' : 'up'}`}>{member.symbol}</Link>
